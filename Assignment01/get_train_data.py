from pathlib import Path
import random
import numpy as np
from tqdm import tqdm
from typing import List
from tokenizers import Tokenizer

# 加载本地的tokenizer
def load_tokenizer(tokenizer_path: str) -> Tokenizer:
    tokenizer = Tokenizer.from_file(tokenizer_path)
    return tokenizer

# 把文本转化为token_id
def text_to_token_id(
    text: str,
    tokenizer: Tokenizer,
    add_eos: bool = True # 在文本后加上结束标志
) -> List[int]:
    encoding = tokenizer.encode(text)
    ids = encoding.ids

    if add_eos:
        eos_id = tokenizer.token_to_id("<|eos|>")
        if eos_id is not None:
            ids.append(eos_id)

    return ids

def build_random_data_bin(
    input_text: str,
    tokenizer_json: str,
    output_bin: str,
    target_samples: int = 10000,
    dtype=np.int32
):
    tokenizer = load_tokenizer(tokenizer_json)

    print("正在扫描文件行数...")
    # 第一遍扫描：获取总行数（用于随机索引）
    with open(input_text, "r", encoding="utf-8") as f:
        total_lines = sum(1 for _ in f)

    print(f"总计 {total_lines} 行， 正在抽取 {target_samples} 个样本...")

    # 随机产生要抽取的行号集合（排好序以便流式抽取）
    target_indices = set(random.sample(range(total_lines), min(target_samples, total_lines)))

    all_ids = []

    # 第二遍扫描：只处理选中的行
    with open(input_text, "r", encoding="utf-8") as f:
        sampled_count = 0 # 记录取到的样本数量
        for i, line in enumerate(tqdm(f, total=total_lines, desc="Processing")):
            if i in target_indices:
                line_stripped = line.strip()
                if not line_stripped:
                    continue

                # 如果这一行仅为<|endoftext|>, 可以转成ID
                ids = text_to_token_id(line_stripped, tokenizer)
                all_ids.extend(ids)

                sampled_count += 1
                # 取够目标样本数后就可以停止了
                if sampled_count >= target_samples:
                    break
    
    # 保存文件
    arr = np.array(all_ids, dtype=dtype)
    arr.tofile(output_bin)
    print(f"成功随机抽取并保存{len(arr):,}tokens到{output_bin}")


if __name__ == "__main__":
    tokenizer_json_path = "./bpe_tokenizer/tokenizer.json"

    build_random_data_bin(
        input_text="./TinyStoriesV2-GPT4-train.txt",
        tokenizer_json=tokenizer_json_path,
        output_bin="data.bin",
        target_samples=800000
    )